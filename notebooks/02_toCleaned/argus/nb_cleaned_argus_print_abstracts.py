...
# MAGIC %md
# MAGIC # 02-cleaned: Abstract und Tonalität Generierung für Argus Print Medien Daten 
# MAGIC
...
# MAGIC Der Output des Modells wird als parquet im 02-cleaned Storage Container gespeichert.
# MAGIC
# MAGIC
# MAGIC ---
# MAGIC * QUELLEN:  
...
# MAGIC - 29.11.2024 Max Mustermann: update api version of OpenAI Model API
# MAGIC - 15.11.2024 Max Mustermann: Code refactoring
# MAGIC - 16.10.2024 Max Mustermann: Update PZ Integration
# MAGIC - 05.09.2024 Max Mustermann: Init
# MAGIC
# MAGIC
# MAGIC
...
pz_source_schema_name = "01_raw"
pz_target_schema_name = "02_cleaned"

# source dir volltexte
source_dir_json = f"argus/print_media_volltexte/{date}/json"
volltext_path = sta_endpoint_pz_uk[pz_source_schema_name] + '/' + source_dir_json

# sink dir abstract
sink_dir_abstract = sta_endpoint_pz_uk[pz_target_schema_name] + '/' + f"argus/print_media_abstracts/{date}/parquet"
    ...
    return [
        {
            ...
            Dies ist der Text:
            Neues Leben für alte Batterien - A und F testen in einem gemeinsamen Projekt, wie alte Antriebsbatterien aus E-Autos weiter genutzt werden können. Dafür wird ein stationärer Batteriespeicher mit Akkus befüllt, die aus zerlegten Testfahrzeugen stammen. Das Aggregat besteht aus zwölf Hochvolt-Batteriesystemen. Zusammengeschaltet bringen sie es auf eine Gesamtleistung von einem MW. In den kommenden Wochen wird zunächst die Leistungsfähigkeit des Speichers geprüft und verschiedene Einsatzszenarien simuliert. Dazu zählt auch die Bereitstellung von Regelleistung für das Stromnetz. Umgekehrt wird bei einem Uberangebot von Wind- oder PV-Anlagen Strom gespeichert. Außerdem wird untersucht, wie die Speicherkapazität am Energiemarkt eingesetzt werden kann, falls günstiger Strom aus erneuerbaren Energien vorhanden ist. In ihrem »zweiten Leben« haben die Zellen nach Ansicht der Experten eine Einsatzzeit von mindestens fünf bis zehn Jahren. Danach führt F die Batterien einem endgültigen Recycling zu. Dabei werden sie in ihre einzelnen Bestandteile und Rohstoffe zerlegt, um perspektivisch wieder in neuen Batterien zum Einsatz zu kommen. Die Anlage dient als Referenz für zunächst vier Projekte, die bei A derzeit für die nähere Zukunft geplant sind.
            """.strip(),
        },
        {
            "role": "assistant", 
            "content": """
            In einer Kooperation zwischen A und F wird die Weiterverwendung alter Antriebsbatterien aus E-Autos in einem stationären Batteriespeicher untersucht. Dieser Speicher, wird zunächst hinsichtlich seiner Leistungsfähigkeit und diverser Einsatzmöglichkeiten getestet. Dazu gehört die Bereitstellung von Regelleistung für das Stromnetz und die Speicherung von Überschussstrom aus erneuerbaren Energiequellen. Ziel ist es auch, die Speicherkapazität gewinnbringend am Energiemarkt zu nutzen. Die Batterien sollen nach ihrer Wiederverwendung, die fünf bis zehn Jahre andauern könnte, durch F recycelt werden. Dieses Projekt dient als Modell für vier weitere Vorhaben, die bei A geplant sind, und ist somit von strategischer Bedeutung für die nachhaltige Energieverwaltung und den Ausbau der Energieeffizienz im Unternehmen.
...
if path_exists(sink_dir_abstract):
    df_abstract = spark.read.parquet(f"{sink_dir_abstract}/{date_path}_print_media_abstracts.parquet")
else:
    df_abstract = spark.createDataFrame([], schema=schema_abstract)
    print("No existing parquet in directory.")
...
# loop over each volltext to generate abstract

if path_exists(volltext_path):
    for volltext in dbutils.fs.ls(volltext_path):
        existing_file = volltext.name.split(".")[0]
        if bool(df_abstract.filter(df_abstract.clipping_id.contains(existing_file)).collect()) == True:
            print(f"Skipping file {volltext.name} because it already exists.")
            continue
        else:
    ...
    print(f"No files found in {volltext_path}.")
...
# MAGIC %md
# MAGIC TODO: Speichern der Abstracts in Tabelle in Cleaned
...
if path_exists(volltext_path):
    df_abstract.write.mode("overwrite").parquet(f"{sink_dir_abstract}/{date_path}_print_media_abstracts.parquet")
    print(f"Saved absracts to {sink_dir_abstract}/{date_path}_print_media_abstracts.parquet.")
else:
