...
path_abstract = sta_endpoint_pz_uk['02_cleaned'] + '/' + f"argus/print_media_abstracts/{date}/parquet"
...
if path_exists(path_abstract):
    df_abstracts = spark.read.parquet(f"{path_abstract}/{date_path}_print_media_abstracts.parquet")
    df_abstracts.createOrReplaceTempView("print_media_abstracts")
else:
    df_abstracts = spark.createDataFrame([], schema=schema_abstract)
    df_abstracts.createOrReplaceTempView("print_media_abstracts")
...
df_print_media_union = spark.sql("""
    ...
    b.DELETED_TS,
    b.OPERATION    
    FROM print_media_abstracts a
    JOIN print_media_cleaned b
    ON a.clipping_id = b.clipping_id
""")

# df_transformed = spark.read.table(pz_target_schema_name+".argus_print_media_panel").select("clipping_id", F.lit(True).alias("is_old"))

# df_print_media_union = df_print_media_union.join(df_transformed, on=["clipping_id"], how="left")

# COMMAND ----------

# Upsert dataframe into derived table
if df_print_media_union.count() > 0:
