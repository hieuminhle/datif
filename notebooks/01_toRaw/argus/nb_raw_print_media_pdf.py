...
# MAGIC %md
# MAGIC # 01-raw: Auslesen der Volltexte aus Print Medien
# MAGIC
...
# MAGIC Getriggert wird der Datenabzug und die Verarbeitung täglich über die ADF Pipeline '00-1000-Argus-orchestrate'.
# MAGIC
# MAGIC
# MAGIC
# MAGIC ---
# MAGIC * QUELLEN:  
# MAGIC -  datif_dz_dev.02_cleaned_uk_argus.argus_print_media_panel_current_view
# MAGIC
# MAGIC * ZIEL:  
# MAGIC Storage: 
# MAGIC - 02-cleaned/argus/print_media_volltexte/YYYY/MM/DD/binary/_clipping_id_.pdf
# MAGIC - 02-cleaned/argus/print_media_volltexte/YYYY/MM/DD/json/_clipping_id_.json
# MAGIC
# MAGIC   
# MAGIC ---
# MAGIC * Versionen (aktuelle immer oben):
# MAGIC - 15.11.2024 Svenja Schuder: code refactoring
# MAGIC - 26.09.2024 Svenja Schuder: Init
...
pz_target_schema_name = "01_raw"

pz_sink_dir_binary = f"argus/print_media_volltexte/{date}/binary"
pz_sink_dir_json = f"argus/print_media_volltexte/{date}/json"

pdf_path = sta_endpoint_pz_uk[pz_target_schema_name] + '/' + pz_sink_dir_binary
json_path = sta_endpoint_pz_uk[pz_target_schema_name] + '/' + pz_sink_dir_json
...
df_print_media_cleaned = spark.sql(f"""
                                   ...
                                   files_pdf_doc
                                   FROM datif_dz_dev.02_cleaned_uk_argus.argus_print_media_panel_current_view
    ...
    if path_exists(pdf_path)==True:
        ...
        for article in result_dict:
            if article['clipping_id'] in existing_files:
                print(f"Skipping {article['clipping_id']} because it already exists in path.")
                continue
            else:
    ...
    if path_exists(json_path)==True:
        ...
        for pdf_file in dbutils.fs.ls(pdf_path):
            if pdf_file.name.split(".")[0] in existing_files:
                print(f"Skipping {pdf_file.name} because it already exists in path.")
                continue
            else:
                ...
                try:
                    ...
                    dbutils.fs.put(sta_endpoint_pz_uk[pz_target_schema_name] + f"/{pz_sink_dir_json}/{clipping_id}.json", json_dumps.decode(), True)
